...
CREATE TABLE IF NOT EXISTS rent_comic(
	...
	rent_user_id INT NOT NULL, CONSTRAINT fk_rent_user_id FOREIGN KEY (rent_user_id) REFERENCES users(user_id) ON UPDATE CASCADE ON DELETE CASCADE,
	rent_comic_id INT NOT NULL, CONSTRAINT fk_rent_comic_id FOREIGN KEY (rent_comic_id) REFERENCES comic(comic_id) ON UPDATE CASCADE ON DELETE CASCADE,
	rent_username VARCHAR(512), CONSTRAINT fk_rent_username FOREIGN KEY (rent_username) REFERENCES users(username) ON UPDATE CASCADE ON DELETE CASCADE,
	rent_comic_name VARCHAR(512), CONSTRAINT fk_rent_comic_name FOREIGN KEY (rent_comic_name) REFERENCES comic(comic_title) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS list_comic(
	...
	list_comic_identifier VARCHAR(512) NOT NULL, CONSTRAINT fk_list_comic_identifier FOREIGN KEY (list_comic_identifier) REFERENCES comic(comic_title) ON UPDATE CASCADE ON DELETE CASCADE,
	list_comic_writer VARCHAR(512) NOT NULL, CONSTRAINT fk_list_comic_writer FOREIGN KEY (list_comic_writer) REFERENCES comic(comic_writer) ON UPDATE CASCADE ON DELETE CASCADE,
